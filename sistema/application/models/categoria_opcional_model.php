...
class Categoria_Opcional_Model extends Abstract_Model {
  
  function __construct() {
    parent::__construct("via_opcionales_categorias","id","nombre ASC");
  }
  ...
  function get($id) {
    ...
    $id = (int)$id;
    $sql = "SELECT R.* ";
    $sql.= "FROM via_opcionales_categorias R ";
    $sql.= "WHERE R.id = $id ";
  ...
  function reorder($elements,$orden = 0, $id_padre = 0) {
    ...
    if (isset($elements["id"])) {
      $id = $elements["id"];
      if (!empty($id)) {
        $sql = "UPDATE via_opcionales_categorias SET orden = $orden, id_padre = $id_padre ";
  ...
  function get_arbol($id_padre = 0,$separador = "") {
    $id_empresa = parent::get_empresa();
    $result = array();
    $q = $this->db->query("SELECT * FROM via_opcionales_categorias WHERE id_empresa = $id_empresa AND id_padre = $id_padre ORDER BY orden ASC");
  ...
  function get_select($id_padre = 0,$separador = "") {
    $id_empresa = parent::get_empresa();
    $result = array();
    $q = $this->db->query("SELECT * FROM via_opcionales_categorias WHERE id_empresa = $id_empresa AND id_padre = $id_padre ORDER BY nombre ASC");
    foreach($q->result() as $row) {
      $e = new stdClass();
      $e->id = $row->id;
      $e->id_padre = $id_padre;
  ...
  function delete($id) {
    ...
    $this->db->query("DELETE FROM via_opcionales_categorias WHERE id = $id AND id_empresa = $id_empresa");
  }
}