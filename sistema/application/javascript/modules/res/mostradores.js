(function ( app ) {
    ...
    app.views.MostradoresTableView = app.mixins.View.extend({

        template: _.template($("#mostradores_resultados_template").html()),
        ...
        myEvents: {
            ...
            "click .buscar":"buscar",
            "click .enviar":"enviar",
            "click .mostradores_tipo_comprobante_check":"buscar",
            "click .nuevo":"nuevo",
        },
        ...
        initialize: function(options) {
            ...
            createdatepicker(this.$("#mostradores_desde"));
            createdatepicker(this.$("#mostradores_hasta"));
            
            this.buscar();
        },
        
        exportar_csv: function(obj) {
            var desde = $("#mostradores_desde").val();
            desde = desde.replace(/\//g,"-");
            var hasta = $("#mostradores_hasta").val();
        ...
        buscar: function() {
            ...
            if (!isEmpty(this.$("#mostradores_listado_buscar").val())) 
                filtros.filter = this.$("#mostradores_listado_buscar").val();
            if (!isEmpty(this.$("#mostradores_listado_cliente").val())) 
                filtros.id_cliente = this.$("#mostradores_listado_cliente").val();
            if (!isEmpty(this.$("#mostradores_listado_numero").val())) 
                filtros.numero = this.$("#mostradores_listado_numero").val();
        ...
        exportar : function() {
            ...
            var fecha_desde = this.$("#mostradores_desde").val();
            if (isEmpty(fecha_desde)) {
                alert("Por favor seleccione una fecha");
                this.$("#mostradores_desde").focus();
                return;
            }
            var fecha_hasta = this.$("#mostradores_hasta").val();
            if (isEmpty(fecha_hasta)) {
                alert("Por favor seleccione una fecha");
                this.$("#mostradores_hasta").focus();
                return;
            }
            
            var url = "/sistema/facturas/function/exportar_excel/?";
            if (!isEmpty(this.$("#mostradores_listado_buscar").val()))
                url+="filter="+this.$("#mostradores_listado_buscar").val()+"&";
            if (!isEmpty(this.$("#mostradores_listado_cliente").val()))
                url+="id_cliente="+this.$("#mostradores_listado_cliente").val()+"&";
            if (!isEmpty(this.$("#mostradores_listado_numero").val()))
                url+="numero="+this.$("#mostradores_listado_numero").val()+"&";
        ...
        addAll : function () {
            this.$("#mostradores_tabla tbody tr").empty();
            this.collection.each(this.addOne);
        },
        
        addOne : function ( item ) {
            var view = new app.views.MostradoresItemResultados({
                model: item,
                seleccionar: this.habilitar_seleccion,
                parent: this.parent,
            });
            this.$("#mostradores_tabla tbody").append(view.render().el);
        },
        
        enviar: function() {
            var checks = this.$("#mostradores_tabla .i-checks input[type=checkbox]:checked");
            if (checks.length == 0) {
...
// -----------------------------------------
(function ( app ) {
    app.views.MostradoresItemResultados = app.mixins.View.extend({
        
        template: _.template($("#mostradores_item_resultados_template").html()),
        tagName: "tr",
        myEvents: {
