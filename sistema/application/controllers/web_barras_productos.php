...
class Web_Barras_Productos extends REST_Controller {

  function __construct() {
    parent::__construct();
    $this->load->model('Web_Barra_Producto_Model', 'modelo',"orden ASC",1);
  }

  function eliminar($id) {
    $id_empresa = parent::get_empresa();
    $sql = "DELETE FROM web_barras_productos WHERE id_empresa = $id_empresa AND id = $id";
    $this->db->query($sql);
  ...
  function ordenar() {
    ...
    if (!empty($ids)) {
      ...
      for($i=0;$i<sizeof($ids);$i++) {
        $id = $ids[$i];
        $this->db->query("UPDATE web_barras_productos SET orden = $i WHERE id = $id AND id_empresa = $id_empresa");
      }
    }
    echo json_encode(array());
  }
