...
class App_Pedidos extends REST_Controller {
  ...
  function replace_placeholder($template,$config = array()) {
    ...
    if (!empty($cliente)) $template->texto = str_replace("{{cliente}}", utf8_decode($cliente), $template->texto);
    $template->texto = str_replace("{{observaciones}}", utf8_decode($observaciones), $template->texto);
    if (!empty($usuario)) {
      $template->texto = str_replace("{{usuario}}", $usuario, $template->texto);
      $template->texto = str_replace("{{comercio}}", $usuario, $template->texto);
    }
    if (!empty($direccion)) $template->texto = str_replace("{{direccion}}", utf8_decode($direccion), $template->texto);
    if (!empty($comercio_direccion)) $template->texto = str_replace("{{comercio_direccion}}", utf8_decode($comercio_direccion), $template->texto);
  ...
  function email_devolucion_dinero() {
    ...
    $body = str_replace("{{nombre}}", utf8_encode($nombre), $body);
