...
class Sindi_Recetarios extends REST_Controller {

  function __construct() {
    parent::__construct();
    $this->load->model('Sindi_Recetario_Model', 'modelo');
  }
  ...
  function imprimir($id) {
    ...
    $this->db->query("UPDATE sindi_recetarios SET impresa = 1 WHERE id = $id AND id_empresa = $id_empresa ");

    $this->load->view("reports/sindi/bonorecetario", array(
      "recetario"=>$datos,
      "titular"=>$titular,
    ));
    ...
    $this->Sindi_Historial_Model->registrar(array(
      ...
      "evento"=>"Bono Recetario",
      "motivo"=>"Impresion Bono Recetario Nro $datos->numero",
    ));
  }
  function buscar() {
    $id_empresa = parent::get_empresa();
